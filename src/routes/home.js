...
module.exports = (HeadComponent, scriptUrl) => (req, res) => {
    ...
    var cookieName = visitor.getCookieName().replace("@","%40");
    var amcvCookie = cookies[cookieName];
    
    console.log(req.query.pageName);
    
    var pagename = "Adobe UK - Server Side";
        if(req.query.pageName)
            pagename+=" - "+req.query.pageName;  
        
    var generateHomePage = generatePage(HomeComponent, HeadComponent, scriptUrl, pagename);
    ...
    fetchTargetedContent(visitor, amcvCookie, function (content) {
        ...
        var pageHtml = generateHomePage(stringify(serverState), stringify(content.payloads), content, pagename);
