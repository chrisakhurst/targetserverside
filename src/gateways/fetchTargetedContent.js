...
module.exports = function fetchTargetedContent(visitor, amcvCookie, callback) {
    ...
    var today = new Date(),
            date = ''+today.getFullYear() + (today.getMonth() + 1) + today.getDate()+'-'+today.getMinutes()+today.getSeconds();
    ...
    var fetchTopLeftBanner = createRequestFor("chrisakhurst-serverside");
    ...
    return Promise.all([fetchTopLeftBanner()])
                .then((content) => {
                    var [topLeft] = content;
                    callback({
                        topLeft: topLeft.content,
